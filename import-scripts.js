(()=> {
  ...
  self.importScriptsSync = function (...sources) {
    let baseUrl;
    try {
      throw new Error('');
    } catch (e) {
    ...
    for (let i = 0; i < sources.length; i += 1) {
      ...
      if (url.origin !== baseUrl.origin) {
        throw new Error('Cross-origin script is not allowed in worker');
      }
      importScript(url, {async: false});
    }
  };
  self.importScriptsAsync = function (...sources) {
    let baseUrl;
    try {
      throw new Error('');
    } catch (e) {
    ...
    for (let i = 0; i < sources.length; i += 1) {
      ...
      if (url.origin !== baseUrl.origin) {
        promises.push(
          Promise.reject(
            new Error('Cross-origin script is not allowed in worker'),
          ),
        );
      } else {
